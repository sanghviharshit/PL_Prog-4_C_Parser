...
//Not used
void FreeTokenData(tokenData *n) {
    if(!n) return;
    if (n->tValue) free(n->tValue);
    free(n);
}

void FreeStateData(curParseState *n) {
	if(!n) return;
    if (n->topToken) {
    	FreeTokenData(n->topToken);
    	free(n->prevToken);
    }
    if (n->prevToken) {
    	FreeTokenData(n->prevToken);
    	free(n->prevToken);
    }
    if (n->lexInput) {
        free(n->prevToken);
...
//				printf("Len: %ld\n",p-startInput);
        ...
        case '#':
            ...
            continue;
		case '.':
			if(parseState->prevToken->tType == T_HOST) {
...
tokenData *Parse(char *input) {
    curParseState parseState;

    startInput = input;
    parseState.lexInput = input;
    parseState.topToken = 0;
    parseState.prevToken = 0;
    LoadNextToken(&parseState);

    return ParseProg(&parseState);
}
